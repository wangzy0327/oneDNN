...
void batch_normalization_example(dnnl::engine::kind engine_kind) {
    ...
    // Create src and scale/shift memory descriptors.
    auto src_md = memory::desc(src_dims, dt::f32, tag::nchw);
    auto scale_shift_md = memory::desc(scale_shift_dims, dt::f32, tag::nc);

    sycl::buffer<float, 3> buffer_src(static_cast<float*>(src_data.data()), sycl::range<3>(N*IC,IH,IW));
    sycl::buffer<float, 2> buffer_scale_shift(static_cast<float*>(scale_shift_data.data()), sycl::range<2>(2, IC));    

    // Create memory objects using make_memory.
    auto src_mem = dnnl::sycl_interop::make_memory(src_md, engine, buffer_src);
    auto scale_shift_mem = dnnl::sycl_interop::make_memory(scale_shift_md, engine, buffer_scale_shift);
    ...
    auto mean_mem = dnnl::sycl_interop::make_memory(bnorm_pd.mean_desc(), engine, dnnl::sycl_interop::memory_kind::buffer);
    auto variance_mem = dnnl::sycl_interop::make_memory(bnorm_pd.variance_desc(), engine, dnnl::sycl_interop::memory_kind::buffer);
    auto workspace_mem = dnnl::sycl_interop::make_memory(bnorm_pd.workspace_desc(), engine, dnnl::sycl_interop::memory_kind::buffer);
