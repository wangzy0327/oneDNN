...
void convolution_example(dnnl::engine::kind engine_kind) {
    ...
    // sycl::buffer<float, 1> buffer_src(static_cast<float*>(src_data.data()), sycl::range<1>(src_data.size()));
    // sycl::buffer<float, 1> buffer_weight(static_cast<float*>(weights_data.data()), sycl::range<1>(weights_data.size()));
    // sycl::buffer<float, 1> buffer_bias(static_cast<float*>(bias_data.data()), sycl::range<1>(bias_data.size()));
    // sycl::buffer<float, 1> buffer_dst(static_cast<float*>(dst_data.data()), sycl::range<1>(dst_data.size()));
    ...
    // auto user_src_mem = memory(src_md, engine);
    auto src_md = memory::desc(src_dims, dt::f32, tag::nhwc);
    auto user_src_mem = memory(src_md, engine);
    // auto user_src_mem = dnnl::sycl_interop::make_memory(src_md, engine, buffer_src);

    // auto user_weights_mem = memory({weights_dims, dt::f32, tag::oihw}, engine);    
    // auto user_weights_mem = memory({weights_dims, dt::f32, tag::hwio}, engine);
    auto weight_md = memory::desc(weights_dims, dt::f32, tag::hwio);    
    auto user_weights_mem = memory(weight_md, engine);
    // auto user_weights_mem = dnnl::sycl_interop::make_memory(weight_md, engine, buffer_weight);
    ...
    auto user_dst_mem = memory(dst_md, engine);    
    // auto user_dst_mem = dnnl::sycl_interop::make_memory(dst_md, engine, buffer_dst);  
    ...
    // auto user_bias_mem = dnnl::sycl_interop::make_memory(user_bias_md, engine, buffer_bias);
