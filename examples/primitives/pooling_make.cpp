...
void pooling_example(dnnl::engine::kind engine_kind) {
    ...
    const memory::dim OH = (IH - ((KH - 1) * DH ) + PH_L + PH_R) / SH + 1;
    const memory::dim OW = (IW - ((KW - 1) * DW ) + PW_L + PW_R) / SW + 1;
    ...
    sycl::buffer<float, 3> buffer_dst(static_cast<float*>(dst_data.data()), sycl::range<3>(N*IC, OH, OW));
    // sycl::buffer<float, 3> buffer_src(static_cast<float*>(src_data.data()), sycl::range<3>(N*IH, IW, IC));
    // sycl::buffer<float, 3> buffer_dst(static_cast<float*>(dst_data.data()), sycl::range<3>(N*OH, OW, IC));    
    ...
    auto workspace_mem = dnnl::sycl_interop::make_memory(pooling_pd.workspace_desc(), engine, dnnl::sycl_interop::memory_kind::buffer);
