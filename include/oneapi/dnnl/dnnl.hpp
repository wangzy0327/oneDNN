...
struct primitive : public handle<dnnl_primitive_t> {
    ...
    enum class kind {
        ...
        concat = dnnl_concat,
        /// A (out-of-place) tensor split primitive.
        split = dnnl_split,
        /// A flatten primitive
        flatten = dnnl_flatten,
...
struct memory : public handle<dnnl_memory_t> {
    ...
    struct desc {
        ...
        desc(const dims &adims, data_type adata_type, format_tag aformat_tag,
            ...
            // TODO: this is added temporarilly
            data.format_tag = convert_to_c(aformat_tag);
            if (!allow_empty)
        ...
        /// Return the format tag when memory descriptor was created with.
        format_tag get_format_tag() const { return static_cast<format_tag>(data.format_tag); }
...
struct post_ops : public handle<dnnl_post_ops_t> {
    ...
    void append_batchnormalization(float epsilon){
        error::wrap_c_api(dnnl_post_ops_append_batchnormalization(get(), epsilon),
                "could not append an batchnormalization post-op");
    }
...
struct primitive_attr : public handle<dnnl_primitive_attr_t> {
    ...
    void get_output_offsets(int &mask, std::vector<float> &offsets) const {
        dnnl_dim_t count;
        int c_mask;
        const float *c_offsets;
        error::wrap_c_api(dnnl_primitive_attr_get_output_offsets(
                                  get(), &count, &c_mask, &c_offsets),
                "could not get output offsets primitive attribute");
        offsets.resize(count);

        mask = c_mask;
        for (dnnl_dim_t c = 0; c < count; ++c)
            offsets[c] = c_offsets[c];
    }

    void set_output_offsets(int mask, const std::vector<float> &offsets) {
        error::wrap_c_api(
                dnnl_primitive_attr_set_output_offsets(
                        get(), (dnnl_dim_t)offsets.size(), mask, offsets.data()),
                "could not set output offsets primitive attribute");
    }
...
struct split : public primitive {
    struct desc {
        /// Underlying C operation descriptor.
        dnnl_split_desc_t data;

        /// Default constructor. Produces an empty object.
        desc() = default;

        desc(const std::vector<memory::desc> &dsts, int split_dimension,
                const memory::desc &src) {
            auto c_dsts = convert_to_c(dsts);
            auto init_status = dnnl_split_desc_init(&data, &src.data, (int)c_dsts.size(), 
                        split_dimension, c_dsts.data());
            error::wrap_c_api(init_status,
                    "could not create a descriptor for a split operation "
                    "primitive");
        }
    };
    ...
    split(const primitive_desc &pd) : primitive(pd.get()) {}
};

// TODO: this primitive is just a shell now, try actually implement this opterator
struct flatten : public primitive {
    struct desc {
        /// Underlying C operation descriptor.
        dnnl_flatten_desc_t data;

        /// Default constructor. Produces an empty object.
        desc() = default;

        desc(const memory::desc &src, const memory::desc &dst) { }
    };
    ...
    flatten(const primitive_desc &pd){ executable = false; }
};
...
inline void primitive::execute(const stream &astream,
    ...
    auto c_primitive = get();
    auto c_stream = astream.get();
    auto c_args_p = c_args.data();
    error::wrap_c_api(dnnl_primitive_execute(c_primitive, c_stream,
                              (int)c_args.size(), c_args_p),
