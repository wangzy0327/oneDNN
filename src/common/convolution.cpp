...
status_t conv_desc_init(convolution_desc_t *conv_desc, prop_kind_t prop_kind,
    ...
    consistency = consistency && (src_desc->ndims == dst_desc->ndims);
    consistency = consistency && utils::one_of(src_desc->ndims, 3, 4, 5);
    consistency = consistency && utils::one_of(
    ...
    // consistency = consistency && (src_desc->dims[0] == dst_desc->dims[0]);
    // consistency = consistency && (src_desc->dims[1] == g * weights_desc->dims[with_groups + 1]);
    // consistency = consistency && (dst_desc->dims[1] == g * weights_desc->dims[with_groups + 0]);
    if (!consistency) return invalid_arguments;
    ...
    if (dilates)
        ...
        utils::array_set(cd.dilates, 1, sp_dims);
