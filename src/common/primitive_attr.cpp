...
status_t offsets_t::set(dim_t count, int mask, const float *offsets) {
    cleanup();

    count_ = count;
    mask_ = mask;

    if (is_runtime_value(*offsets)) {
        offsets_ = offsets_buf_;
        offsets_[0] = *offsets;
    } else if (count_ == 1) {
        offsets_ = offsets_buf_;
        utils::array_set(offsets_, offsets[0], offsets_buf_size);
    } else {
        offsets_ = (float *)impl::malloc(count_ * sizeof(*offsets_), 64);
        if (offsets_ == nullptr) return status::out_of_memory;

        for (dim_t c = 0; c < count_; ++c)
            offsets_[c] = offsets[c];
    }
...
status_t post_ops_t::append_batchnormalization(float epsilon) {
    if (len() == post_ops_limit) return out_of_memory;

    entry_.emplace_back();
    auto &e = entry_.back();
    e.kind = primitive_kind::batch_normalization;
    e.batchnorm.epsilon = epsilon;
    return success;
}
    ...
    for (int idx = 0; idx < len(); ++idx) {
        ...
        if (kind == primitive_kind::sum) {
            ...
            if (is_runtime_value(e.scale) || is_runtime_value(e.alpha)
                ...
                return false;
        } else if (kind == primitive_kind::batch_normalization) {
            if (is_runtime_value(entry_[idx].batchnorm.epsilon)) return false;
...
status_t dnnl_primitive_attr_get_output_offsets(const primitive_attr_t *attr,
        dim_t *count, int *mask, const float **offsets) {
    if (any_null(attr, count, mask, offsets)) return invalid_arguments;

    *count = attr->output_offsets_.count_;
    *mask = attr->output_offsets_.mask_;
    *offsets = attr->output_offsets_.offsets_;

    return success;
}

status_t dnnl_primitive_attr_set_output_offsets(
        primitive_attr_t *attr, dim_t count, int mask, const float *offsets) {
    bool ok = !any_null(attr, offsets) && count > 0 && mask >= 0
            && attr->scales_.has_default_values()
            && IMPLICATION(is_runtime_value(*offsets), count == 1);
    if (!ok) return invalid_arguments;

    return attr->output_offsets_.set(count, mask, offsets);
}
...
status_t dnnl_post_ops_append_batchnormalization(post_ops_t *post_ops, float epsilon) {
    if (post_ops == nullptr) return invalid_arguments;
    return post_ops->append_batchnormalization(epsilon);
}
