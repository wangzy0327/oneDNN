...
// beta
struct offsets_t : public c_compatible {
    offsets_t() : count_(1), mask_(0), offsets_(offsets_buf_) { set(0.); }
    offsets_t(dim_t count, int mask, const float *offsets)
        : offsets_(offsets_buf_) {
        set(count, mask, offsets);
    }

    ~offsets_t() { cleanup(); }

    bool operator==(const offsets_t &rhs) const {
        bool ret = count_ == rhs.count_ && mask_ == rhs.mask_
                && !utils::any_null(offsets_, rhs.offsets_)
                && defined() == rhs.defined()
                && IMPLICATION(defined(),
                        !std::memcmp(
                                offsets_, rhs.offsets_, sizeof(float) * count_));
        return ret;
    }

    bool has_default_values() const {
        for (dim_t c = 0; c < count_; ++c) {
            if (offsets_[c] != 1.) return false;
        }
        return true;
    }

    bool defined() const { return !is_runtime_value(offsets_[0]); }

    status_t set(dim_t count, int mask, const float *offsets);
    status_t set(float single_offset) { return this->set(1, 0, &single_offset); }

    status_t copy_from(const offsets_t &other) {
        return set(other.count_, other.mask_, other.offsets_);
    }
...
private:
    enum { offsets_buf_size = 16 };
    float offsets_buf_[offsets_buf_size];

    void cleanup() {
        if (offsets_ != offsets_buf_ && offsets_ != nullptr) impl::free(offsets_);
    ...
    DNNL_DISALLOW_COPY_AND_ASSIGN(offsets_t);
};
...
struct dnnl_post_ops : public dnnl::impl::c_compatible {
    struct entry_t {
        ...
        union {
            ...
            eltwise_t eltwise;
            batchnormalization_t batchnorm;
        ...
        bool operator==(const entry_t &rhs) const {
            ...
            switch (kind) {
                ...
                case primitive_kind::batch_normalization:
                    ret = batchnorm.epsilon == rhs.batchnorm.epsilon;
                    break;
    ...
    dnnl::impl::status_t append_batchnormalization(
            float epsilon);
...
struct dnnl_primitive_attr : public dnnl::impl::c_compatible {
    ...
    dnnl::impl::offsets_t output_offsets_;
