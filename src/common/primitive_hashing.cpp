    ...
    for (int i = 0; i < attr.post_ops_.len(); i++) {
        ...
        switch (entry.kind) {
            ...
            case primitive_kind::batch_normalization:
                seed = hash_combine(seed, entry.batchnorm.epsilon);
                break;
...
size_t get_desc_hash(const split_desc_t &desc) {
    size_t seed = 0;
    // Kinds
    seed = hash_combine(seed, static_cast<size_t>(desc.primitive_kind));
    // Constant args
    seed = hash_combine(seed, desc.n);
    seed = hash_combine(seed, desc.split_dimension);
    // Memory descriptors
    seed = hash_combine(seed, get_md_hash(desc.src_md));
    seed = get_array_hash(seed, desc.dst_mds, desc.n);
    // Combined hash for split op desc
    return seed;
}
