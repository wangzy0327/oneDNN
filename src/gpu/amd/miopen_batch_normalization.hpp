...
struct miopen_batch_normalization_common_t {
    ...
    static void init_ws(const pd_t *pd, memory_desc_t &ws_md) {
        ...
        const dims_t ws_size = {(dim_t)(y_size * pd->fuse_norm_relu() + mean_invvar_size)};

        dnnl_memory_desc_init_by_tag(&ws_md, 1, ws_size, wrap.data_type(), format_tag::x);
    }
};
    ...
    struct pd_t : public batch_normalization_fwd_pd_t {
        ...
        status_t init(engine_t *) {
            ...
            if (use_global_stats()) {
                ...
                bnorm_impl_.reset(new miopen_batch_normalization_fwd_stats_impl_t());
            } else {
...
struct miopen_batch_normalization_bwd_t : public primitive_t {
    ...
    struct pd_t : public batch_normalization_bwd_pd_t {
        ...
        status_t init(engine_t *) {
            ...
            if (fuse_norm_relu()) {
                bnorm_impl_.reset(new miopen_batch_normalization_bwd_relu_impl_t());
            } else {
