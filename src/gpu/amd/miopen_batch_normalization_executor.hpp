...
protected:
    ...
    void interop_task_fwd(
            ...
            amd::sycl_hip_stream_t *hip_stream, read_acc_t src_acc,
        ...
        std::shared_ptr<::sycl::accessor<uint8_t, 1, ::sycl::access::mode::write>> wkspace_acc;
        ...
        maybe_init_mean_var(hip_stream, mean_acc, var_acc, init_mean_var);
        maybe_init_ss(hip_stream, scale_acc, bias_acc, init_ss);
    ...
    template <typename T>
    void maybe_init_ss(
            amd::sycl_hip_stream_t *hip_stream, T, T, bool) const {}

    template <typename T>
    void maybe_init_ss(amd::sycl_hip_stream_t *hip_stream,
    ...
    template <typename T>
    void maybe_init_mean_var(amd::sycl_hip_stream_t *hip_stream,
...
struct bnorm_exec_fwd_inf_t : public bnorm_exec_base_t {
    status_t execute(const exec_ctx_t &ctx, engine_t *engine,
        ...
        return hip_stream->interop_task([&](::sycl::handler &cgh) {
            ...
            interop_task_fwd(bnorm_impl, engine, cgh, hip_stream, src_acc,
...
struct bnorm_exec_bwd_t : public bnorm_exec_base_t {
    status_t execute(const exec_ctx_t &ctx, engine_t *engine,
        ...
        return hip_stream->interop_task([&](::sycl::handler &cgh) {
            ...
            interop_task_bwd(bnorm_impl, engine, cgh, hip_stream, src_acc,
