...
struct miopen_batch_normalization_impl_base_t {
    virtual ~miopen_batch_normalization_impl_base_t() {
        for (size_t i = 0; i < NUM_IO; ++i) {
            if (tensor_descs_[i]) {
                MIOPEN_EXECUTE_FUNC_V(miopenDestroyTensorDescriptor, tensor_descs_[i]);
            }
        }
    ...
    virtual void execute(miopenHandle_t handle, std::shared_ptr<bnorm_args_t> args) const = 0;
...
protected:
    ...
    virtual status_t create_and_set_scaleshift_desc() {
        CHECK(MIOPEN_EXECUTE_FUNC_S(miopenCreateTensorDescriptor, &tensor_descs_[scl]));
    ...
    virtual status_t create_and_set_activation_desc() {
        CHECK(MIOPEN_EXECUTE_FUNC_S(miopenCreateActivationDescriptor, &act_desc_));
        CHECK(MIOPEN_EXECUTE_FUNC_S(miopenSetActivationDescriptor, act_desc_,
                miopenActivationRELU, relu_coef_, 0, 0));

        return status::success;
    }

    virtual status_t to_population_variance(miopenHandle_t handle, void *var) const {
        CHECK(MIOPEN_EXECUTE_FUNC_S(miopenScaleTensor, handle, tensor_descs_[scl],
    ...
    double factor_ = 1.0;
    double eps_ = 0.00001;  // set epsilon with a small figure (greater than 0)
...
// computes mean and variance during propagation
struct miopen_batch_normalization_fwd_impl_t
    ...
    void execute(miopenHandle_t handle, std::shared_ptr<bnorm_args_t> args) const override {
        auto fwd_args = static_cast<bnorm_fwd_args_t *>(args.get());

        float v_alpha = alpha_;
        float v_beta = beta;
        // printf("mean:%p, var:%p, saved_mean:%p, saved_var:%p\n", fwd_args->mean_, fwd_args->var_, fwd_args->save_mean_, fwd_args->save_var_);
        // printf("a1:%f, b1:%f, a2:%f, b2:%f\n", v_alpha, v_beta, *(float*)fwd_args->scale_, *(float*)fwd_args->bias_);
        // printf("scale length:%d\n", sizeof(fwd_args->scale_));
        MIOPEN_EXECUTE_FUNC(miopenBatchNormalizationForwardTraining, handle,
                mode_, &v_alpha, &v_beta, tensor_descs_[src], fwd_args->x_,
...
protected:
    void do_relu(miopenHandle_t handle, bnorm_fwd_args_t *fwd_args) const {
        if (is_training_ && fuse_norm_relu_) {
            ...
            float alpha2 = 0;
            MIOPEN_EXECUTE_FUNC(miopenOpTensor, handle, miopenTensorOpAdd, &alpha_,
                    tensor_descs_[dst], fwd_args->y_, &alpha2, tensor_descs_[dst],
                    fwd_args->y_, &beta, tensor_descs_[dst], fwd_args->y_prime_);
        }
...
// use user provided mean and variance 
    ...
    ~miopen_batch_normalization_bwd_impl_t() {
        for (size_t i = 0; i < NUM_DIFF; i++) {
            if (diff_tensor_descs_[i]) {
                MIOPEN_EXECUTE_FUNC_V(miopenDestroyTensorDescriptor, diff_tensor_descs_[i]);
            }
        }
    }
...
struct miopen_batch_normalization_bwd_relu_impl_t
    ...
    void execute(miopenHandle_t handle,
        ...
        MIOPEN_EXECUTE_FUNC(miopenActivationBackward, handle, act_desc_, &alpha_,
