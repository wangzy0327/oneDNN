...
inline bool reorder_check(const memory_desc_wrapper &src_d,
    ...
    using namespace utils;
    // TODO: delete some unimplement format(miopen only support nchw for 4d tensor)
    return true
            && (
                (src_d.matches_tag(nwc) && (wei_d.matches_one_of_tag(oiw, iwo) != undef))
                || (src_d.matches_tag(ncw) && (wei_d.matches_one_of_tag(wio, owi) != undef))
                || (src_d.matches_tag(nchw) && (wei_d.matches_one_of_tag(ohwi, hwio) != undef))
                || (src_d.matches_tag(ndhwc) && (wei_d.matches_one_of_tag(oidhw, idhwo) != undef))
                || (src_d.matches_tag(ncdhw) && (wei_d.matches_one_of_tag(odhwi, dhwio) != undef))
            )
...
inline bool dense_check(const memory_desc_wrapper &src_d,
        const memory_desc_wrapper &wei_d, const memory_desc_wrapper &dst_d) {
    return true && src_d.is_dense(true) && dst_d.is_dense() && wei_d.is_dense(true);
}
...
struct miopen_gemm_inner_product_fwd_t : public miopen_inner_product_fwd_t {
    ...
    struct pd_t : public parrent_pd_t {
        ...
        status_t init(engine_t *engine) {
            ...
            bool gemm_compatible = gemm_consitency_check(src_md(), weights_md(), dst_md());
