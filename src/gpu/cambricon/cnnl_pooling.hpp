#ifndef GPU_CAMBRICON_CNNL_POOLING_HPP
#define GPU_CAMBRICON_CNNL_POOLING_HPP
...
#include "gpu/cambricon/cnnl_pooling_impl.hpp"
...
struct cnnl_pooling_common_t {
    ...
    void init_ws(const pd_t *pd, memory_desc_t &ws_md) {
        ...
        const dims_t ws_size = {(dim_t)(src_size + dst_size)};  // TODO: unconfirmed
...
struct cnnl_pooling_fwd_t : public primitive_t {
    using primitive_t::primitive_t;
    struct pd_t : public pooling_fwd_pd_t, public cnnl_pooling_common_t {
        using pooling_fwd_pd_t::pooling_fwd_pd_t;

        DECLARE_COMMON_PD_T("bang:cnnls:any", cnnl_pooling_fwd_t);
        ...
        status_t init(engine_t *engine) {
            ...
            ok = ok
                    && IMPLICATION(utils::one_of(src_dt, f16),
                            desc()->prop_kind == forward_inference);    // TODO: cudnn doesn't support f16 forward training, cnnl?
            ok = ok
                    && IMPLICATION(
            ...
            pooling_impl_.reset(new cnnl_pooling_fwd_impl_t());
            return pooling_impl_->init(engine, this);
        }
        ...
        bool blocking_ok() const {
            ...
            if (utils::one_of(src_md()->data_type, data_type::s8)
                ...
                return memory_desc_matches_nhwc_vect_c(src_md())
                        && memory_desc_matches_nhwc_vect_c(dst_md());
            }

            return true;
        }

        std::shared_ptr<cnnl_pooling_impl_base_t> pooling_impl_;
    };
...
struct cnnl_pooling_bwd_t : public primitive_t {
    using primitive_t::primitive_t;
    struct pd_t : public pooling_bwd_pd_t, public cnnl_pooling_common_t {
        using pooling_bwd_pd_t::pooling_bwd_pd_t;

        DECLARE_COMMON_PD_T("bang:cnnl:any", cnnl_pooling_bwd_t);
        ...
        status_t init(engine_t *engine) {
            ...
            pooling_impl_.reset(new cnnl_pooling_bwd_impl_t());
            return pooling_impl_->init(engine, this);
        }
