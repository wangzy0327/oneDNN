...
public:
    status_t init(const reorder_pd_t *pd) override {
        ...
        convert_dims(pd->dst_md()->padded_dims, dims_dst, pd->dst_md()->ndims);
        convert_dims(pd->src_md()->padded_dims, dims_src, pd->src_md()->ndims);               
        ...
        CHECK(CNNL_EXECUTE_FUNC_S(cnnlSetTensorDescriptor, src_desc_, src_format_, src_data_type_, ndims_, dims_src));
       
        // Create and set destination tensor descriptor
        CHECK(CNNL_EXECUTE_FUNC_S(cnnlCreateTensorDescriptor, &dst_desc_));           
        CHECK(CNNL_EXECUTE_FUNC_S(cnnlSetTensorDescriptor, dst_desc_, dst_format_, dst_data_type_, ndims_, dims_dst));
        return status::success;
    }
    ...
    bool layout_n_permute(const reorder_pd_t *pd) {
        ...
        else if(src_format_ == cnnlTensorLayout_t::CNNL_LAYOUT_NCDHW){
            if(dst_format_ == cnnlTensorLayout_t::CNNL_LAYOUT_NCDHW) 
                permute = {0, 1, 2, 3, 4};
            else if(dst_format_ == cnnlTensorLayout_t::CNNL_LAYOUT_NDHWC)
                permute = {0, 2, 3, 4, 1};
            else
                return false;
        }
        else if(src_format_ == cnnlTensorLayout_t::CNNL_LAYOUT_NDHWC){
            if(dst_format_ == cnnlTensorLayout_t::CNNL_LAYOUT_NDHWC) 
                permute = {0, 1, 2, 3, 4};
            else if(dst_format_ == cnnlTensorLayout_t::CNNL_LAYOUT_NCDHW)
                permute = {0, 4, 1, 2, 3};
            else
                return false;
        }
