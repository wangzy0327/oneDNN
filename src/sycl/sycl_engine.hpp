...
bool is_cambricon_gpu(const ::sycl::device &dev);
...
inline std::vector<::sycl::device> get_sycl_devices(
    ...
    const uint32_t vendor_id
            = ((dev_type == ::sycl::info::device_type::gpu) ? 0x10DE : intel_vendor_id);
#endif

#ifdef DNNL_SYCL_HIP
    const uint32_t vendor_id
            = ((dev_type == ::sycl::info::device_type::gpu) ? 0x10DE : intel_vendor_id);    // some where is wrong, so just use nvidia's id
    ...
    devices.erase(std::remove_if(devices.begin(), devices.end(),
                          [=](const ::sycl::device &dev) {
                              auto _vendor_id = dev.get_info<
                                      ::sycl::info::device::vendor_id>();   //4098 vs. 4318 ???
